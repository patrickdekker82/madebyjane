...
export const materialStatuses = {
  undecided: "Nog te kiezen",
  proposed: "Voorgesteld",
  sample_requested: "Monster aangevraagd",
  chosen: "Gekozen",
  client_confirmed: "Door klant bevestigd",
  replaced: "Vervangen",
} as const;
export const quantityBasisLabels = {
  floor_area: "Netto vloeroppervlak",
  wall_area: "Netto wandoppervlak",
  perimeter: "Netto omtrek",
  plinth: "Plintlengte (omtrek min deuren)",
} as const;
...
export const sampleStatuses = {
  none: "Geen monster",
  requested: "Monster aangevraagd",
  received: "Monster ontvangen",
  approved: "Monster goedgekeurd",
  rejected: "Monster afgewezen",
} as const;
const date = z
  .string()
  .regex(/^\d{4}-\d{2}-\d{2}$/)
...
const decimal = z
  ...
  .regex(
    /^(?:0|[1-9]\d{0,6})(?:\.\d{1,3})?$/,
    "Gebruik een positief aantal met maximaal drie decimalen.",
  );
const percent = z
  .string()
  .regex(
    /^(?:0|[1-9]\d?)(?:\.\d{1,2})?$/,
    "Gebruik een opslag tussen 0 en 99,99 procent.",
  );
const step = z
  .string()
  .regex(/^(?:0|[1-9]\d{0,4})(?:\.\d{1,3})?$/)
  .refine(
    (value) => Number(value) > 0,
    "Een bestelstap moet groter dan nul zijn.",
  );
...
const productFields = {
  ...
  /**
   * De kleur zoals de ontwerper hem op het monster ziet, alleen voor de
   * weergave. Bewust een eigen veld naast `colorCode`: een leverancierscode
   * als "RAL 9010" is niet betrouwbaar naar een schermkleur te rekenen, en een
   * gegokte kleur in een klantbeeld is erger dan geen kleur. Ontbreekt hij, dan
   * blijft het vlak in 3D neutraal en zegt de weergave dat erbij.
   */
  displayColor: z
    .string()
    .regex(/^#[0-9a-fA-F]{6}$/, "Gebruik een kleur als #a1b2c3.")
    .nullable()
    .default(null),
};
type Product = {
...
const productRules = (value: Product, ctx: z.RefinementCtx) => {
  if (
    ...
    (!value.priceSource || value.priceDate === null)
  )
    ctx.addIssue({
      code: "custom",
      path: ["priceSource"],
...
export const quantityRequestSchema = z
  .object({
    variantId: id,
...
export const quantitySourceSchema = quantityRequestSchema
  .extend({
...
const definitionFields = {
  ...
  ...productFields,
  sampleStatus: z
    .enum(["none", "requested", "received", "approved", "rejected"])
    .default("none"),
...
const sharedRules = (value: Fields, ctx: z.RefinementCtx) => {
  ...
  if (value.sampleStatus === "none" && value.sampleDate !== null)
    ctx.addIssue({
      code: "custom",
      path: ["sampleDate"],
      message: "Een monsterdatum hoort bij een monsterstatus.",
    });
  if (value.sampleStatus !== "none" && value.sampleDate === null)
    ctx.addIssue({
      code: "custom",
      path: ["sampleDate"],
      message: "Leg vast op welke datum deze monsterstatus geldt.",
    });
  if (value.status === "sample_requested" && value.sampleStatus === "none")
    ctx.addIssue({
      ...
      path: ["sampleStatus"],
      message:
        "Zet ook de monsterstatus wanneer de keuze op Monster aangevraagd staat.",
    });
  if (
    new Set(value.alternatives.map((a) => a.id)).size !==
    value.alternatives.length
  )
    ctx.addIssue({
      code: "custom",
      path: ["alternatives"],
      message: "Elk alternatief heeft een eigen ID.",
    });
  if (
    value.status === "client_confirmed" &&
    (!value.confirmationDate || !value.confirmationNote)
  )
    ctx.addIssue({
      code: "custom",
      path: ["confirmationNote"],
      message: "Leg datum en bron van het klantakkoord vast.",
    });
  if (
    value.status !== "client_confirmed" &&
    (value.confirmationDate !== null || value.confirmationNote)
  )
    ctx.addIssue({
      code: "custom",
      path: ["confirmationNote"],
      message: "Klantakkoord hoort alleen bij de status Door klant bevestigd.",
    });
};
const manualQuantityRule = (value: Fields, ctx: z.RefinementCtx) => {
  if (value.quantity !== null && !value.quantityReason)
    ctx.addIssue({
      code: "custom",
      path: ["quantityReason"],
      message: "Onderbouw de handmatig ingevoerde hoeveelheid.",
    });
};
/** Wat de client instuurt: zonder berekende bron, die vult de server in. */
export const materialDefinitionInputSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.unit !== value.calculation.unit)
      ctx.addIssue({
        code: "custom",
        path: ["unit"],
        message: "De eenheid moet bij de berekende hoeveelheid passen.",
      });
    if (value.quantity === null)
      ctx.addIssue({
        code: "custom",
        path: ["quantity"],
        message: "Een berekende keuze heeft altijd een hoeveelheid.",
      });
    else if (
      value.quantity !== value.calculation.orderQuantity &&
      !value.quantityReason
    )
      ctx.addIssue({
        code: "custom",
        path: ["quantityReason"],
        message: "Onderbouw waarom je afwijkt van de berekende hoeveelheid.",
      });
  });
export const materialPublishSchema = z
  .object({
    entryId: id,
    versionId: id,
...
export type MaterialDefinitionInput = z.infer<
  typeof materialDefinitionInputSchema
>;
...
export function withDefaults(
  ...
  return {
    ...
    displayColor: definition.displayColor ?? null,
